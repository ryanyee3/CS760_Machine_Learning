...
\begin{tikzpicture}
    \node [mybox] (box){%
        \begin{minipage}{0.3\textwidth}
            ...
            Input: $x$, Weights: $w$, Bias: $b$, Activation Function: $r$ \\
            Output: $r(w^{T}x + b)$ which feeds into the next layer \\
            Gradient Components: $\frac{\partial l}{\partial w} = (\hat y-y)x$, $\frac{\partial l}{\partial x} = (\hat y-y)w$ \\
            2-Layer: $\frac{\partial l}{\partial w_{11}} = \frac{\partial l}{\partial a_{11}}\frac{\partial a_{11}}{\partial w_{11}^{(1)}} = (\hat y-y)w_{11}^{(2)} a_{11}(1-a_{11})x_{1}$
        \end{minipage}
    };
